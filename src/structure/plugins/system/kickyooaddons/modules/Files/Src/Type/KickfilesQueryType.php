				    ...
				    'metadata' => [
					    ...
					    'fields' => [
						    'pattern' => [
                                ...
                                'description' => "Pick a folder to load file content dynamically. Alternatively, set a path using a <a href=\"https://www.php.net/manual/en/function.glob.php\" target=\"_blank\">glob pattern</a> to filter files. For example: <code>{$rootDir}/*.{jpg,png}</code>. The path is relative to the system folder and must be a subdirectory of <code>{$rootDir}</code>.<br>You can use:<br><ul><li><code>{id}</code> for the Article ID.</li><li><code>{alias}</code> for the Article Alias.</li><li><code>{jcf name}</code> or <code>{jcf name,value}</code> for custom fields (if no value is specified, rawvalue is taken).</li><li><code>{intro_image}</code> for the folder name derived from the article\'s Intro Image.</li><li><code>{image_fulltext}</code> for the folder name derived from the article\'s Full Text Image.</li></ul><br>For example, if the Intro Image URL is <code>{$rootDir}/articles/intro/sample.jpg</code>, using <code>{intro_image}</code> will set the folder name to <code>{$rootDir}/articles/intro</code>. Ensure that the resulting path complies with system security requirements and remains a subdirectory of <code>{$rootDir}</code>",
                                'type' => 'select-file',
                            ],
						    '_offset' => [
							    'description' => trans(
